  ...
  for (let p = start; p <= end; p++) {
    try {
      await new Promise<void>((resolve, reject) => {
        const s = http.createServer(() => { /* noop */ });
        s.once('error', reject);
    ...
    try {
      const ok = await new Promise<boolean>((resolve) => {
        const req = http.request(base + '/healthz', (res) => {
          ...
          res.on('end', () => {
            try { const j = JSON.parse(Buffer.concat(chunks).toString('utf-8')); resolve(!!j?.ok); }
            catch { resolve(false); }
          });
        });
...
async function main() {
  ...
  const server = spawn(process.execPath, ['dist-run/scripts/server.js', `--port=${port}`], { stdio: 'inherit' });
  ...
  const smoke = spawn(process.execPath, ['dist-run/scripts/smoke_http.js'], { stdio: 'inherit', env });
