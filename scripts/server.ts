...
const stakeSmokePath = resolve(root, "stake-smoke.html");
...
// Optional external sounds directory override
const soundsDirArg =
  process.argv
    .slice(2)
    .find((a: string) => a.startsWith("--soundsDir="))
    ?.split("=")[1] || process.env.SOUNDS_DIR;
const soundsDir = soundsDirArg ? resolve(resolveEnvLike(soundsDirArg) || "") : "";
...
const server = http.createServer(async (req, res) => {
  ...
  // Map external sounds: serve /assets/sounds/* from --soundsDir or SOUNDS_DIR when provided
  if ((method === "GET" || method === "HEAD") && path.startsWith("/assets/sounds/")) {
    try {
      if (soundsDir && existsSync(soundsDir)) {
        // sanitize requested relative path
        const rel = decodeURIComponent(path.substring("/assets/sounds/".length)).replace(
          /\\/g,
          "/"
        );
        ...
        const candidate = resolve(soundsDir, rel);
        if (!candidate.startsWith(resolve(soundsDir))) {
          res.writeHead(403, { "content-type": "text/plain; charset=utf-8" });
        ...
        if (existsSync(candidate)) {
          ...
          if (!st.isDirectory()) {
            ...
            if (method === "GET") createReadStream(candidate).pipe(res);
            else res.end();
            return;
          }
        }
        // Fall through to static if file missing
      }
    } catch {}
    // Default: try static dist-web
  ...
  // Serve the host smoke page from repo root for convenience
  if (method === "GET" && (path === "/stake-smoke" || path === "/stake-smoke.html")) {
    try {
      if (!existsSync(stakeSmokePath)) {
        res.writeHead(404, { "content-type": "text/plain; charset=utf-8" });
        return res.end("stake-smoke.html not found");
      }
      const html = readFileSync(stakeSmokePath, "utf-8");
      res.writeHead(200, {
      ...
      res.writeHead(500, { "content-type": "text/plain; charset=utf-8" });
      return res.end("failed to load stake-smoke.html");
    }
  }
...
function parsePort(v: unknown, fallback: number): number {
  ...
  const n =
    typeof raw === "string" && raw.length > 0 ? Number(raw) : typeof raw === "number" ? raw : NaN;
...
function tryListen(startPort: number, attempts = 5) {
  ...
  server.on("listening", () => {
    // eslint-disable-next-line no-console
    console.log(`[server] listening on host:port ${host}:${current} serving ${publicDir}`);
  });
  server.listen(current, host);
}
