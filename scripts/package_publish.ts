...
function main() {
  ...
  if (!src) {
    ...
    // Fallback candidates in repo if no games/* detected
    if (!src) {
      const artifactsMath = resolve(root, "artifacts", "math_static");
      if (existsSync(join(artifactsMath, "index.json"))) src = artifactsMath;
    }
    if (!src) {
      const distPublish = resolve(root, "dist-publish");
      if (existsSync(join(distPublish, "index.json"))) src = distPublish;
    }
  }
  ...
  if (src && existsSync(join(src, "index.json"))) {
    ...
    if (py.status !== 0) {
      throw new Error(
        "Failed to generate publish files. Ensure Python and requirements are installed."
      );
    }
