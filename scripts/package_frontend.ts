...
const embedDemoHtml = `<!doctype html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>POCKET MUNSTERS (Embed Demo)</title>
  <link rel="icon" href="data:," />
  <link rel="stylesheet" href="styles.css?v=${VERSION}" />
  <link rel="stylesheet" href="ui.css?v=${VERSION}" />
  <link rel="stylesheet" href="bg.css?v=${VERSION}" />
  <meta http-equiv="Content-Security-Policy" content="default-src 'self'; img-src 'self' data:; style-src 'self' 'unsafe-inline'; script-src 'self'; connect-src 'self'" />
  <meta name="description" content="POCKET MUNSTERS embed demo" />
  <meta name="robots" content="noindex,nofollow" />
  <meta http-equiv="X-Content-Type-Options" content="nosniff" />
  <meta http-equiv="Referrer-Policy" content="no-referrer" />
</head>
<body>
  ...
  <script src="embed.js?v=${VERSION}" defer></script>
</body>
</html>`;
...
// --- Auto-mount embed.min.js ---
const embedMinJsTemplate = (version: string, stylesCss: string, uiCss: string, bgCss: string, symbolsModuleContent: string, formatJs: string, soundsJs: string, animJs: string, appJs: string) => {
  // Minified: injects markup and CSS into #pm-embed
  const markup = `<div class=\"game-root\"><div id=\"loader\" class=\"loader\" aria-hidden=\"true\" hidden><div class=\"spinner\"></div><div class=\"label\">Loading…</div></div><section class=\"game-stage\" aria-label=\"Game stage\"><canvas id=\"stage\" width=\"896\" height=\"512\" aria-label=\"Game canvas\"></canvas><div id=\"overlay\" class=\"stage-overlay\" aria-hidden=\"true\"></div></section><aside class=\"hud\" aria-label=\"Game HUD\"><div class=\"hud-row\"><div class=\"stat\"><span>Balance</span><strong id=\"balance\">$1,000.00</strong></div><div class=\"stat\"><span>Bet</span><div class=\"bet-ctrl\"><button id=\"betDec\" class=\"btn sm\" aria-label=\"Decrease bet\">−</button><input id=\"bet\" type=\"number\" min=\"1\" step=\"1\" value=\"1\" inputmode=\"numeric\" /><button id=\"betInc\" class=\"btn sm\" aria-label=\"Increase bet\">+</button></div></div><div class=\"stat\"><span>Win</span><strong id=\"win\">$0.00</strong></div><div class=\"stat\"><span>Currency</span><select id=\"currencySel\" class=\"sel\" aria-label=\"Currency\"><option value=\"USD\">USD</option><option value=\"EUR\">EUR</option><option value=\"JPY\">JPY</option><option value=\"BRL\">BRL</option><option value=\"INR\">INR</option></select></div><div class=\"stat\"><span>Language</span><select id=\"langSel\" class=\"sel\" aria-label=\"Language\"><option value=\"en\">English</option></select></div><div class=\"spacer\"></div><div class=\"controls\"><button id=\"rulesBtn\" class=\"btn ghost\">Rules</button><button id=\"paytableBtn\" class=\"btn ghost\">Paytable</button><button id=\"muteBtn\" class=\"btn ghost\" aria-pressed=\"false\">🔊</button><button id=\"autoBtn\" class=\"btn ghost\" aria-pressed=\"false\" title=\"Autoplay (asks to confirm)\">Auto</button><button id=\"spinBtn\" class=\"btn primary\" title=\"Spacebar to spin\">Spin</button></div></div></aside><section id=\"messages\" class=\"messages\" aria-live=\"polite\" aria-atomic=\"true\"></section><dialog id=\"confirmAuto\" class=\"modal\"><form method=\"dialog\"><h2>Enable Autoplay?</h2><p>Autoplay will place spins consecutively until stopped.</p><menu><button value=\"cancel\" class=\"btn ghost\">Cancel</button><button value=\"ok\" class=\"btn primary\">Enable</button></menu></form></dialog><dialog id=\"rulesModal\" class=\"modal\"><form method=\"dialog\"><h2>Game Rules</h2><div id=\"rulesBody\" class=\"modal-body\"></div><menu><button value=\"close\" class=\"btn primary\">Close</button></menu></form></dialog><dialog id=\"paytableModal\" class=\"modal\"><form method=\"dialog\"><h2>Paytable</h2><div id=\"paytableBody\" class=\"modal-body\"></div><menu><button value=\"close\" class=\"btn primary\">Close</button></menu></form></dialog></div>`;
  const css = `<style>${stylesCss}\n${uiCss}\n${bgCss}</style>`;
  // Minify helpers
  const stripExports = s => s.replace(/^export\s+default\s+/mg, '').replace(/^export\s+const\s+/mg, 'const ').replace(/^export\s+function\s+/mg, 'function ').replace(/^export\s+class\s+/mg, 'class ');
  const stripImports = s => s.replace(/^import\s+[^;]+;\s*\n?/mg, '');
  ...
  appCore = appCore.replace(/\n?const \$\s*=\s*\(sel\)\s*=>\s*document\.querySelector\(sel\);\s*/,'').replace(/\n?const stage\s*=\s*\$('#stage');\s*/,'').replace(/\n?const animator\s*=\s*new\s+Animator\(stage\);\s*/,'');
  return `/*! Pocket Munsters auto-mount embed; v${version} */\n(function(){\n'use strict';\nfunction mount(){\n  var root = document.getElementById('pm-embed');\n  if (!root) return;\n  root.innerHTML = '${css}${markup}';\n  var stage = root.querySelector('#stage');\n  if (!stage) return;\n  var animator = new Animator(stage);\n${appCore}\n}\nwindow.PocketMunstersAuto = { mount, Animator, version: '${version}' };\nif (document.readyState!=='loading') mount(); else document.addEventListener('DOMContentLoaded', mount, { once: true });\n})();\n`;
};
html,body{height:100%}
...
`})();\n`;
  ...
  // Write auto-mount embed.min.js
  try {
    const bgCss = bgCssTemplate(findLikelyBackground(distWeb) || 'assets/bg.jpg');
    const embedMinJs = embedMinJsTemplate(VERSION, stylesCss, uiCss, bgCss, symbolsModuleContent, formatJs, soundsJs, animJs, appJs);
    writeFileSync(resolve(distWeb, 'embed.min.js'), embedMinJs);
    console.log('[package:web] embed.min.js (auto-mount) written');
  } catch (e) {
    console.warn('[package:web] embed.min.js generation failed:', e);
  }
  } catch (e) {
    console.warn('[package:web] embed.js generation failed:', e);
  }

  // Write a demo HTML that uses embed.js so we can verify integration quickly
  try {
    writeFileSync(resolve(distWeb, 'embed-demo.html'), embedDemoHtml);
    console.log('[package:web] embed-demo.html written');
  } catch (e) {
    console.warn('[package:web] failed to write embed-demo.html:', e);
  }
}
