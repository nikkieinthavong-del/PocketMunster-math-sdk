...
function main() {
  ...
  if (!distRunEngine) {
    throw new Error(`Compiled engine not found. Run "npm run build:demo" first or pass ENGINE_PATH/arg.`);
  }
  ...
  console.log(`[package:math] Prepared dist-math with engine.js, index.json${existsSync(configSrc) ? ', config.json' : ''}.`);
}
