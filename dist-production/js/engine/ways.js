 ...
 */
export function calculateWaysWins(grid, paytable, // symbol -> count -> payout
multiplierMap, config = DEFAULT_WAYS_CONFIG) {
    ...
    for (const [symbolKey, positions] of symbolGroups.entries()) {
        if (positions.length < config.minSymbolsForWin)
            continue;
...
function calculateDirectionalWays(positions, rows, cols, direction, config) {
    ...
    const sortedColumns = Array.from(byColumn.keys()).sort((a, b) => direction === 'leftToRight' ? a - b : b - a);
...
function checkEvolutionChainWin(grid) {
    ...
    const hasCompleteChain = evolutionChains.some(chain => chain.every(species => speciesFound.has(species)));
