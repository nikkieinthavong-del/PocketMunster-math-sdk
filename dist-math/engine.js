...
export function spin(configJson, bet, opts = {}) {
    ...
    const multipliers = opts.initMultiplierMap
        ? opts.initMultiplierMap.map((row) => row.slice())
        : Array.from({ length: rows }, () => Array.from({ length: cols }, () => 0));
    const events = [];
    events.push({ type: 'spinStart', payload: { seed: opts.seed } });
    // Helpers
    const tierMapCfg = (configJson?.evolution?.tiers ?? undefined);
    const reverseTierToBase = new Map();
    if (tierMapCfg) {
        for (const base of Object.keys(tierMapCfg)) {
            const tiers = tierMapCfg[base] || {};
            for (const k of Object.keys(tiers)) {
                const symId = String(tiers[k] ?? '').toLowerCase();
                if (symId)
                    reverseTierToBase.set(symId, base);
            }
        }
    }
    const normalizeId = (id) => {
        ...
        if (low === '__empty__' || low === 'wild' || low === 'freespins' || low === 'freeSpins'.toLowerCase() || low === 'pokeball' || low === 'masterball' || low === 'masterBall'.toLowerCase() || low === 'egg')
            return id;
    ...
    const markEmpty = (r, c) => (grid[r][c].id = '__empty__');
    ...
    const findClusters = () => {
        ...
        const clusters = [];
        const dfs = (r, c, targetBase, acc) => {
            if (!inBounds(r, c) || seen[r][c])
                return;
            const curRaw = grid[r][c].id;
            const cur = curRaw;
            if (cur === '__empty__')
                return;
            const curBase = normalizeId(String(cur));
            if (!(curBase === targetBase || cur === 'wild'))
                return;
            seen[r][c] = true;
            acc.push([r, c]);
            for (const [nr, nc] of neighbors4(r, c))
                dfs(nr, nc, targetBase, acc);
        };
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                ...
                if (curId === '__empty__' || curId === 'wild' || curId === 'freeSpins' || curId === 'pokeball' || curId === 'masterBall' || curId === 'egg') {
                    seen[r][c] = true;
                    continue;
                }
                const targetBase = normalizeId(curId);
                const acc = [];
                dfs(r, c, targetBase, acc);
                if (acc.length >= MIN_CLUSTER)
                    clusters.push({ id: targetBase, cells: acc });
            }
        }
    ...
    const scatterCounts = { freeSpins: 0, pokeball: 0 };
    for (let r = 0; r < rows; r++)
        for (let c = 0; c < cols; c++) {
    ...
    while (cascade < maxCascades) {
        ...
        if (inBonus === 'hunt' && wildsPerCascade > 0) {
            ...
            if (chosen.length) {
                events.push({ type: 'wildInject', payload: { index: cascade, positions: chosen.map(([r, c]) => ({ row: r, col: c })) } });
            }
        }
        ...
        for (const cl of clusters) {
            ...
            const base = basePays[cl.id] * size * baseFactor; // apply baseFactor from config
            // Aggregate multiplier factor: treat 0 as x1 baseline, otherwise multiply xN
            const factor = cl.cells.reduce((acc, [r, c]) => acc * (multipliers[r][c] > 0 ? multipliers[r][c] : 1), 1);
            ...
            const evo = checkEvolution(grid, cl.cells, neighbors4, (configJson?.evolution?.tiers ?? undefined));
            if (evo.events.length) {
                for (const ev of evo.events)
                    events.push(ev);
            }
            // Remove only the cells in the winning cluster (no all-of-type removal)
            for (const [r, c] of cl.cells) {
                if (grid[r][c].id !== '__empty__') {
                    markEmpty(r, c);
    ...
    const outGrid = Array.from({ length: rows }, (_, r) => Array.from({ length: cols }, (_, c) => ({ id: grid[r][c].id })));
    return {
        grid: outGrid,
