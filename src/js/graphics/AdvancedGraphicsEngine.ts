...
export class AdvancedGraphicsEngine {
  ...
  private renderSymbol(symbol: any, row: number, col: number, program: WebGLProgram) {
    ...
    gl.uniform3f(
      glowColorUniform,
      pokemonData.glowColor[0],
      pokemonData.glowColor[1],
      pokemonData.glowColor[2]
    );
  ...
  private renderMultiplierEffect(
    multiplier: number,
    row: number,
    col: number,
  ...
  // Utility methods
  private getPokemonData(symbolId: string): {
    glowColor: [number, number, number];
    brightness: number;
    rarity: string;
  } {
    const pokemonDatabase: Record<
      string,
      { glowColor: [number, number, number]; brightness: number; rarity: string }
    > = {
  ...
  private drawQuad(x: number, y: number, width: number, height: number) {
    ...
    const vertices = new Float32Array([x, y, x + width, y, x, y + height, x + width, y + height]);
