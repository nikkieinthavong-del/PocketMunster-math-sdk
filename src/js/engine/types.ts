...
export interface Cell {
  kind: Kind;
  tier: 1 | 2 | 3 | 4 | 5 | 0; // 0 for non-standard specials
  // id groups standard matching (e.g., "tier1_A"). For now, we group by "tierN_X".
  id: string;
}
...
export interface SpinConfig {
  rows: number;
  cols: number;
  weights: Record<string, number>; // from config.symbolWeights
