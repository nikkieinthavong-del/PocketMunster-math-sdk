...
export interface WaysConfig {
  minSymbolsForWin: number;
  payLeftToRight: boolean;
  payRightToLeft: boolean;
  adjacentOnly: boolean; // true for adjacent ways, false for any-position ways
  maxWays: number; // 7^7 = 823,543 theoretical max for 7x7, but 117,649 for 7x5
}
...
const DEFAULT_WAYS_CONFIG: WaysConfig = {
  minSymbolsForWin: 3,
  payLeftToRight: true,
...
export function calculateWaysWins(
  grid: Grid,
  paytable: Record<string, Record<number, number>>, // symbol -> count -> payout
  multiplierMap?: number[][],
  config: WaysConfig = DEFAULT_WAYS_CONFIG
): WaysResult {
  ...
  const symbolGroups = new Map<string, Array<{ pos: [number, number]; tier: number }>>();
  ...
  for (const [symbolKey, positions] of symbolGroups.entries()) {
    ...
    if (config.payLeftToRight) {
      const leftToRightWin = calculateDirectionalWays(
        positions,
        rows,
        cols,
        'leftToRight',
    ...
    if (config.payRightToLeft) {
      const rightToLeftWin = calculateDirectionalWays(
        positions,
        rows,
        cols,
        'rightToLeft',
...
function calculateDirectionalWays(
  positions: Array<{ pos: [number, number]; tier: number }>,
  rows: number,
  cols: number,
  direction: 'leftToRight' | 'rightToLeft',
  config: WaysConfig
): { ways: number; symbolCount: number; positions: Array<[number, number]> } {
...
function getPayoutForSymbol(
  ...
  const basePayout = basePayouts[symbolCount as keyof typeof basePayouts] || 0;
  const tierMultiplier = tierMultipliers[tier as keyof typeof tierMultipliers] || 1;
 ...
 */
export function calculateMegaWays(
  grid: Grid,
  paytable: Record<string, Record<number, number>>,
  multiplierMap?: number[][],
  config: WaysConfig = DEFAULT_WAYS_CONFIG
  ...
  megaWins: Array<{
    type: 'evolution_chain' | 'full_screen' | 'all_tiers';
    multiplier: number;
    positions: Array<[number, number]>;
    bonusAmount: number;
  }>;
...
function checkEvolutionChainWin(grid: Grid): { isWin: boolean; positions: Array<[number, number]> } {
...
function checkFullScreenWin(grid: Grid): { isWin: boolean; positions: Array<[number, number]> } {
  const positions: Array<[number, number]> = [];
  let firstSpecies: string | null = null;
  let allSame = true;
