...
export interface RTOOptimizationResult {
  targetRTP: number;
  achievedRTP: number;
  variance: number;
  ...
  recommendedPaytables: PaytableSet;
  featureContributions: FeatureContribution;
  simulationResults: SimulationSummary;
}

export interface PaytableSet {
  basePays: Record<string, Record<number, number>>;
  clusterBonuses: Record<number, number>;
  evolutionMultipliers: Record<string, number>;
  freeSpinMultipliers: Record<number, number>;
  megaWinMultipliers: Record<string, number>;
}

export interface FeatureContribution {
  basePays: number;        // % of total RTP
  clusterPays: number;     // % of total RTP
  waysPays: number;        // % of total RTP
  tumbleMechanics: number; // % of total RTP
  evolutionSystem: number; // % of total RTP
  freeSpins: number;       // % of total RTP
  megaFeatures: number;    // % of total RTP
}
...
export interface SimulationSummary {
  ...
  totalWins: number;
  biggestWin: number;
  rtpByDenomination: Record<string, number>;
  hitFrequencyByFeature: Record<string, number>;
  averageFeatureWin: Record<string, number>;
}

export class RTOOptimizer {
  private targetRTP: number;
  private volatilityTarget: 'low' | 'medium' | 'high' | 'extreme';
  private simulationSpins: number;

  constructor(targetRTP: number = 0.945, volatility: 'low' | 'medium' | 'high' | 'extreme' = 'high') {
   ...
   */
  public optimizePaytables(): RTOOptimizationResult {
  ...
  private generateBasePaytables(): PaytableSet {
  ...
  private iterativeOptimization(initialPaytables: PaytableSet): PaytableSet {
  ...
  private adjustPaytables(paytables: PaytableSet, factor: number): PaytableSet {
    const adjusted: PaytableSet = {
      basePays: {},
      clusterBonuses: {},
      evolutionMultipliers: {},
  ...
  private runQuickSimulation(paytables: PaytableSet, spins: number): SimulationSummary {
    let totalWins = 0;
    let biggestWin = 0;
    ...
    return {
      ...
      hitFrequencyByFeature: Object.fromEntries(
        Object.entries(hitsByFeature).map(([feature, hits]) => [feature, hits / spins])
      ),
      averageFeatureWin: Object.fromEntries(
        Object.entries(hitsByFeature).map(([feature, hits]) => [feature, hits > 0 ? totalWins / hits : 0])
      ),
    };
  }

  private runComprehensiveSimulation(paytables: PaytableSet): SimulationSummary {
    return this.runQuickSimulation(paytables, this.simulationSpins);
  }

  private simulateSingleSpin(paytables: PaytableSet): {
    totalWin: number;
    featureHits: Record<string, boolean>;
  } {
    let totalWin = 0;
    const featureHits: Record<string, boolean> = {
      basePays: false,
      clusterPays: false,
      waysPays: false,
      evolution: false,
    ...
    if (random < baseHitFreq) {
      ...
      let tier = 'tier1';
      if (tierRandom < 0.1) tier = 'tier3';
      else if (tierRandom < 0.25) tier = 'tier2';

      const sizeRandom = Math.random();
      let symbolCount = 3;
      if (sizeRandom < 0.05) symbolCount = 7;
      else if (sizeRandom < 0.15) symbolCount = 6;
      else if (sizeRandom < 0.30) symbolCount = 5;
      else if (sizeRandom < 0.55) symbolCount = 4;
  ...
  private calculateFeatureContributions(simulation: SimulationSummary): FeatureContribution {
  ...
  private calculateVariance(simulation: SimulationSummary): number {
    ...
    if (maxWinRatio > 2000) return 4.0; // Extreme
    if (maxWinRatio > 1000) return 3.0; // Very High
    if (maxWinRatio > 500) return 2.0;  // High
    if (maxWinRatio > 200) return 1.0;  // Medium
    return 0.5; // Low
  }

  private classifyVolatility(simulation: SimulationSummary): 'low' | 'medium' | 'high' | 'extreme' {
    const variance = this.calculateVariance(simulation);

    if (variance >= 3.5) return 'extreme';
    if (variance >= 2.5) return 'high';
    if (variance >= 1.5) return 'medium';
    return 'low';
  }

  private calculateHitFrequency(simulation: SimulationSummary): number {
   ...
   */
  public static generateBalancedPaytables(targetRTP: number): PaytableSet {
   ...
   */
  public static validateRTPCompliance(paytables: PaytableSet, requiredRTP: number = 0.92): {
    isCompliant: boolean;
    actualRTP: number;
   ...
   */
  public static generateMarketSpecificPaytables(): Record<string, PaytableSet> {
    return {
