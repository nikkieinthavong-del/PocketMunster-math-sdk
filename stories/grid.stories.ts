...
const meta: Meta<GridArgs> = {
  ...
  theme: { control: { type: 'select' }, options: ['light','dark'] },
    ...
    wins: { control: 'object' },
    winsMode: { control: { type: 'select' }, options: ['manual','none','ring','plus','random'] },
    ...
    animEvo: { control: { type: 'select' }, options: ['pulse','ascend','shine','rotate','none'] },
    ...
    for (const url of candidates) {
      try {
        ...
        const meta: AssetsMeta = {
          ...
          names: Array.isArray(data.names) ? data.names : Array.isArray(data.sprites?.names) ? data.sprites.names : undefined,
          backNames: Array.isArray(data.backNames) ? data.backNames : Array.isArray(data.back?.names) ? data.back.names : undefined,
        };
        if (meta.baseDir) {
          cachedAssets = meta;
          return cachedAssets;
...
function sanitizeName(s: string){ return String(s||'').replace(/[^A-Za-z0-9_\-]/g,''); }
...
function createGrid(root?: HTMLElement, opts?: { 
  cells?: MockCell[][];
  wins?: WinsPoint[];
  // look & layout
  size?: number; gap?: number; rows?: number; cols?: number; winColor?: string; showLabels?: boolean; showMultipliers?: boolean; useWallpaper?: boolean;
  // content
  useSprites?: boolean; arenaBacks?: boolean; seed?: string; symbolSource?: 'auto' | 'fallback' | 'manifest';
  // mults
  multMode?: 'all1' | 'edge2x' | 'random'; multMin?: number; multMax?: number;
  // wins mode
  winsMode?: 'manual' | 'none' | 'ring' | 'plus' | 'random'; winsCount?: number;
  // animations
  animSpin?: 'wobble'|'flash'|'shake'|'pop'|'none';
  animWin?: 'glow'|'bounce'|'pop'|'twist'|'none';
  animEvo?: 'pulse'|'ascend'|'shine'|'rotate'|'none';
  ...
  ['asel-spin-wobble','asel-spin-flash','asel-spin-shake','asel-spin-pop','asel-win-glow','asel-win-bounce','asel-win-pop','asel-win-twist','asel-evo-pulse','asel-evo-ascend','asel-evo-shine','asel-evo-rotate']
  ...
  const cells = opts?.cells ?? defaultCells({
    rows, cols,
    seed: opts?.seed ?? 'demo',
  ...
  const p = typeof opts?.evoChance === 'number' ? Math.max(0, Math.min(1, opts.evoChance)) : 0.02;
  ...
  legend.textContent = (opts?.useSprites ? 'Sprites mode (served from /assets) — ' : 'Pure mock (colors only) — ') + '"win" cells are highlighted.';
...
function defaultCells(opts: { rows: number; cols: number; seed: string; symbolSource: 'auto'|'fallback'|'manifest'; preferSprites: boolean; multMode: 'all1'|'edge2x'|'random'; multMin: number; multMax: number; }): MockCell[][] {
  ...
  let pool = fallback;
  const wantsManifest = opts.symbolSource === 'manifest' || (opts.symbolSource === 'auto' && opts.preferSprites);
  ...
  const out: MockCell[][] = Array.from({ length: R }, (_, r) => Array.from({ length: C }, (_, c) => {
...
function generateWins(rows: number, cols: number, mode: 'none'|'ring'|'plus'|'random', count: number, rng: () => number): WinsPoint[] {
  const pts: WinsPoint[] = [];
  const R = Math.max(1, Math.floor(rows));
  const C = Math.max(1, Math.floor(cols));
  if (mode === 'none') return pts;
  if (mode === 'ring') {
    for (let c = 0; c < C; c++) { pts.push({ row: 0, col: c }); pts.push({ row: R-1, col: c }); }
    for (let r = 1; r < R-1; r++) { pts.push({ row: r, col: 0 }); pts.push({ row: r, col: C-1 }); }
...
export const Default: StoryObj<GridArgs> = {
  ...
  render: (args) => createGrid(undefined, {
    size: args.size,
    gap: args.gap,
    rows: args.rows,
    cols: args.cols,
