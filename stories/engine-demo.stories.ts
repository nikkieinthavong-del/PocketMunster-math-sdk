...
function renderSpin(seed: number, bet: number, cascades: number, showMults: boolean, mode: Args['mode'], rushTarget: number, wildPerCascade: number){
  ensureStyles();
  const root = document.createElement('div'); root.className = 'demo-wrap';
  const config = { grid: { rows: 6, cols: 5 }, multipliers: { cellMax: 8192 }, engine: { chanceAddMultiplier: .3, chanceMasterBall: .15, hunt: { rushTarget, wildPerCascade } } };
  const res = spin(config as any, bet, { seed, maxCascades: cascades, inBonusMode: mode });
  const injected = res.events.filter(e => e.type==='wildInject').flatMap((e:any) => e.payload.positions as Array<{row:number; col:number}>);
  const grid = document.createElement('div'); grid.className = 'grid';
  const rows = res.grid.length, cols = res.grid[0].length;
  grid.style.gridTemplateColumns = `repeat(${cols}, 64px)`;
  for (let r=0;r<rows;r++) for (let c=0;c<cols;c++){
    const el = document.createElement('div'); el.className='cell';
    const id = (res.grid as any)[r][c]?.id;
    const lab = document.createElement('div'); lab.className='lab'; lab.textContent = String(id);
    el.appendChild(lab);
    if (showMults){ const m = res.multiplierMap[r][c] || 0; if (m>0){ const t=document.createElement('div'); t.className='mult'; t.textContent = 'x'+m; el.appendChild(t);} }
    if (mode==='hunt' && injected.some(p => p.row===r && p.col===c)){
      const inj = document.createElement('div'); inj.className='inj'; inj.title='wild injected'; el.appendChild(inj);
    }
    grid.appendChild(el);
  }
  ...
  if ((res.uiHints as any)?.rush){
    ...
    const bar = document.createElement('div'); bar.style.width = Math.min(100, Math.floor(progress/Math.max(1,target)*100)) + '%';
  ...
  res.events.forEach((e) => {
    ...
    if (e.type==='win') line.textContent = `win: ${e.payload.symbol.id} size=${e.payload.size} x=${e.payload.multiplier} amount=${e.payload.winAmount}`;
    else if (e.type==='masterBall') line.textContent = `masterBall: x${e.payload.multiplier}`;
    else if (e.type==='wildInject') line.textContent = `wildInject: cascade=${(e.payload as any).index} positions=${(e.payload as any).positions.length}`;
  ...
  root.appendChild(grid); root.appendChild(events);
  return root;
}
...
export const Demo: StoryObj<Args> = {
  ...
  render: (args) => renderSpin(args.seed, args.bet, args.cascades, args.showMultipliers, args.mode, args.rushTarget, args.wildPerCascade)
};
