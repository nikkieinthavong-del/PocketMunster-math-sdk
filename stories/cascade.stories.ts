...
function makeRng(seed: string){
  ...
  return function() { h = Math.imul(h ^ (h >>> 16), 2246822507); h = Math.imul(h ^ (h >>> 13), 3266489909); const t = (h ^= h >>> 16) >>> 0; return (t & 0xfffffff) / 0x10000000; };
}
function colorFor(name: string): string { let h = 0; for (let i=0;i<name.length;i++) h = (h*31 + name.charCodeAt(i))>>>0; const hue=h%360; return `linear-gradient(135deg, hsl(${hue} 65% 60%/.95), hsl(${(hue+40)%360} 70% 45%/.95))`; }
...
function buildGrid(rows: number, cols: number, rng: () => number): Cell[][] {
  ...
  const g: Cell[][] = Array.from({ length: rows }, () => Array.from({ length: cols }, () => ({ empty: true } as Cell)));
...
function findClusterWins(grid: Cell[][], strategy: 'cross' | 'largestCluster'): Array<[number, number]> {
  const rows = grid.length, cols = grid[0]?.length ?? 0;
  if (strategy === 'cross') {
    const cr = Math.floor(rows/2), cc = Math.floor(cols/2);
  return ([ [cr, cc] as [number, number], [cr-1, cc] as [number, number], [cr+1, cc] as [number, number], [cr, cc-1] as [number, number], [cr, cc+1] as [number, number] ]).filter(([r,c]) => r>=0 && r<rows && c>=0 && c<cols);
  }
  ...
  for (let c=0;c<cols;c++) {
    ...
    for (let r=rows-1;r>=0;r--) {
      if (!('empty' in grid[r][c])) {
        if (write !== r) { grid[write][c] = grid[r][c]; grid[r][c] = { empty: true } as Cell; }
        write--;
      }
    }
...
function renderWithArgs(args: Args) {
  ...
  const autoLabel = document.createElement('label'); autoLabel.style.display = 'inline-flex'; autoLabel.style.alignItems = 'center'; autoLabel.style.gap = '6px';
  const autoChk = document.createElement('input'); autoChk.type = 'checkbox'; autoChk.checked = !!args.autoLoop;
  autoLabel.appendChild(autoChk); autoLabel.appendChild(document.createTextNode('Auto'));
  const resetBtn = document.createElement('button'); resetBtn.textContent = 'Reset';
  const gridEl = document.createElement('div');
  let running = false; let current = 0; let grid = buildGrid(args.rows, args.cols, rng);
  renderGrid(gridEl, grid, args.showLabels);
  gridEl.style.setProperty('--cell-size', '72px');
  gridEl.style.setProperty('--cell-gap', '6px');
  toolbar.appendChild(playBtn); toolbar.appendChild(stepBtn); toolbar.appendChild(autoLabel); toolbar.appendChild(resetBtn);
  wrap.appendChild(toolbar); wrap.appendChild(gridEl);
  ...
  async function run() {
    if (running) return; running = true; current = 0;
  ...
  function reset() {
    running = false; current = 0;
    grid = buildGrid(args.rows, args.cols, makeRng(args.seed || 'demo'));
    gridEl.classList.remove('spinning');
    renderGrid(gridEl, grid, args.showLabels);
  }

  playBtn.addEventListener('click', () => { if (!running) run(); });
  stepBtn.addEventListener('click', () => { if (!running) oneCascade(); });
