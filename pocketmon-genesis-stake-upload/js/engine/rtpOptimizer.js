...
export class RTOOptimizer {
  ...
  optimizePaytables() {
    console.log(
      `Optimizing paytables for ${(this.targetRTP * 100).toFixed(1)}% RTP with ${this.volatilityTarget} volatility`,
    );
  ...
  iterativeOptimization(initialPaytables) {
    ...
    while (iterations < maxIterations && bestRTPError > 0.001) {
      ...
      iterations++;
      console.log(
        `Iteration ${iterations}: RTP = ${(currentRTP * 100).toFixed(3)}%, Error = ${(rtpError * 100).toFixed(3)}%`,
      );
    }
  ...
  runQuickSimulation(paytables, spins) {
    ...
    return {
      ...
      averageFeatureWin: Object.fromEntries(
        Object.entries(hitsByFeature).map(([feature, hits]) => [
          feature,
          hits > 0 ? totalWins / hits : 0,
        ]),
      ),
    };
  }
  ...
  simulateSingleSpin(paytables) {
    ...
    if (random < baseHitFreq) {
      ...
      else if (sizeRandom < 0.3) symbolCount = 5;
  ...
  calculateFeatureContributions(simulation) {
    ...
    return {
      ...
      waysPays: 0.1 * totalRTP, // 10% from ways pays
