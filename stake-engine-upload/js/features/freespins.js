...
import { MIN_GRID_ROWS, MIN_GRID_COLS, MIN_PROGRESSIVE_LEVEL, MAX_PROGRESSIVE_LEVEL, MAX_MULTIPLIER, SEED_MIX_XOR, SEED_ROTATE_13, SEED_ROTATE_17, SEED_ROTATE_5, RNG_MULTIPLIER, RNG_INCREMENT, RNG_MODULUS, HIGH_SCATTER_BONUS_MULTIPLIER, PROGRESSIVE_BONUS_SPIN_MULTIPLIER, LEGENDARY_MODE_MULTIPLIER, DEFAULT_FREESPINS_CONFIG } from '../engine/constants.js';
// ============================================================================
// ERROR CLASSES
// ============================================================================
...
function validateGridDimensions(rows, cols) {
    ...
    if (rows < MIN_GRID_ROWS || cols < MIN_GRID_COLS) {
        throw new InvalidGridDimensionsError(`Grid dimensions too small. Minimum: ${MIN_GRID_ROWS}x${MIN_GRID_COLS}`, { rows, cols, minRows: MIN_GRID_ROWS, minCols: MIN_GRID_COLS });
    }
    if (rows > 20 || cols > 20) {
        throw new InvalidGridDimensionsError('Grid dimensions too large. Maximum: 20x20', { rows, cols });
    }
}
/**
...
function validateConfig(config) {
    ...
    for (let i = 0; i < config.progressionThresholds.length; i++) {
        if (typeof config.progressionThresholds[i] !== 'number' || config.progressionThresholds[i] < 0) {
            throw new InvalidConfigurationError(`Invalid progression threshold at index ${i}`, { threshold: config.progressionThresholds[i], index: i });
        }
    }
}
/**
...
function applyProgressionBoost(multiplierMap, level) {
    if (level < MIN_PROGRESSIVE_LEVEL || level > MAX_PROGRESSIVE_LEVEL) {
        throw new FreeSpinsError(`Invalid progressive level: ${level}. Must be between ${MIN_PROGRESSIVE_LEVEL} and ${MAX_PROGRESSIVE_LEVEL}`, 'INVALID_PROGRESSIVE_LEVEL', { level, min: MIN_PROGRESSIVE_LEVEL, max: MAX_PROGRESSIVE_LEVEL });
    }
...
function applyProgressiveMultiplierToPositions(multiplierMap, level, positions) {
    if (level < MIN_PROGRESSIVE_LEVEL || level > MAX_PROGRESSIVE_LEVEL) {
        throw new FreeSpinsError(`Invalid progressive level: ${level}`, 'INVALID_PROGRESSIVE_LEVEL', { level });
    }
    ...
    for (const [row, col] of positions) {
        if (row >= 0 && row < multiplierMap.length && col >= 0 && col < multiplierMap[row].length) {
            multiplierMap[row][col] = Math.min(multiplierMap[row][col] * clampedMultiplier, MAX_MULTIPLIER);
        }
    }
}
/**
...
export function stepFreeSpins(state, configJson) {
    ...
    try {
        ...
        // Process features
        const { evolutionBoostActive, evolutionSteps } = processEvolutionBoost(grid, rng, config, state, newProgressiveLevel);
        const { morphingChanceBoost, morphingSteps } = processEnhancedMorphing(grid, rng, config, state, newProgressiveLevel);
        ...
        // Process retriggers
        const { spinsToAdd, newRetriggerCount, shouldActivateLegendary } = processRetrigger(grid, rng, config, newProgressiveLevel, state.retriggerCount);
        ...
        if (shouldActivateLegendary) {
            ...
            for (let row = 0; row < newMultiplierMap.length; row++) {
                for (let col = 0; col < newMultiplierMap[row].length; col++) {
                    newMultiplierMap[row][col] = Math.min(newMultiplierMap[row][col] * LEGENDARY_MODE_MULTIPLIER, MAX_MULTIPLIER);
                }
            }
        }
        ...
        throw new FreeSpinsError(`Unexpected error during free spins step: ${error instanceof Error ? error.message : 'Unknown error'}`, 'STEP_PROCESSING_ERROR', { originalError: error });
    }
}
/**
